...
while run:
    ...
    for e in event.get():
        if e.type == QUIT:
            run = False
    if game:
        rectcheck1 = draw.rect(back, (15, 15, 0), Rect(0, 0, 40, 10))
        rectcheck2 = draw.rect(back, (15, 15, 0), Rect(660, 0, 40, 10))
        rect1 = draw.rect(back, (15, 15, 190), Rect(0, 0, reversey, 10))
        rect2 = draw.rect(back, (15, 15, 190), Rect(660, 0, return2, 10))

        if reversey <= 40 :
            reversey+=0.1
        if return2 <=40:
            return2+=0.1
        if reversey >= 40 and keys[K_d]:
            yspeed*=-1
            reversey = 0



        if return2 >=40 and keys[K_RIGHT]:
            ballr.speed*=-1
            return2 = 0
        if p1losescore ==7:
