...
while run:
    ...
    if game:
        ...
        if ballr.rect.x <= 0:
            ballr.speed *= -1
            p2losescore+=1
        if not keys[K_TAB] and p1count>0:
            p1count-=1

        if keys[K_TAB]:
            p1count+=0.1

        if not keys[K_SPACE] and p2count >= 0:
            p2count -= 1
        if keys[K_SPACE]:
            p2count += 0.1
        if ballr.rect.colliderect(p1) and keys[K_TAB] and ballr.speed<0 and p1count<1.1:

            ballr.speed *= -1.02
            p1.speed *= 1.03


        if ballr.rect.colliderect(p2) and keys[K_SPACE] and ballr.speed>0 and p2count<1.1:
            ballr.speed *= -1.02
            p2.speed *= 1.03
